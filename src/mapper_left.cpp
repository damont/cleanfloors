...
void Mapper_left::move_and_update(Mapper_info& m_info, int32_t num_steps, State state)
{

	int32_t added_steps = 0;

	// Search in the deque and get the Path we need.
	for (std::deque<Path>::reverse_iterator itty = m_info.m_map.rbegin();
			(itty != m_info.m_map.rend()) && (added_steps < num_steps);
			++itty)
...
void Mapper_left::update_current(Mapper_info& m_info, State state)
{

}

void Mapper_left::update_front(Mapper_info& m_info, State state)
{
...
int32_t Mapper_left::next_object(Mapper_info& m_info)
{
...
void Mapper_left::move(Mapper_info& m_info, int32_t num_steps)
{
